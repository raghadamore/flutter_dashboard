...
class ArgriculturalProjectFormController extends GetxController {
  ServicesViewController controller=ServicesViewController.instance;
  TableController controller1=TableController.instance;
  static ArgriculturalProjectFormController get instance=>Get.find();
...
final int index=  Get.arguments;
  ...
  void onInit() {
   ...
   void getData(int index) {
    AgriculturalRepository().getDetailProject(index: index,).then((value) {
      value.fold(
   ...
   void patchCommericalProject(int index,String status,String delivary_date) {
    AgriculturalRepository().patchCommericalProject(index: index,status: status,delivary_date: delivary_date).then((value) {
      value.fold(
        (l) { BotToast.showText(text: l);
        sccess.value=false;},
        (r) {
